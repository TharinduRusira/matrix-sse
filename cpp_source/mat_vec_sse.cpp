...
/*
void mat_vec_mul_sse(size_t n, float vec_c[n],float mat[n][n],float b[n]){
  // n is divisible by 4 in our experiments, so no need of a sanity check
*/
...
void mat_vec_mul_sse(size_t N, size_t M, float* avec, float** amat)
{
  ...
  cout << "time for mat_vec_mul_sse = " << clock() - start << endl; 
}
